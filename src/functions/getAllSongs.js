...
import { query, where, orderBy, collection, Timestamp, getDocs } from "firebase/firestore";  
...
const createSongLinkedList = async (items, scores, storage, setSongList, setCurrentSong, setTodasLasCanciones) => {
  ...
  items.forEach((itemRef) => {
    ...
    .then((url) => {
      ...
      songNodes.push(new Node(itemRef._location.bucket, itemRef._location.path_, url))
      itemsProcessed++;
...
const sortByRanking = (items, rankings) => items
...
const getAllSongs = async(storage, setSongList, setCurrentSong, setTodasLasCanciones, setListas) => {
  ...
  // Crear la lista enlazada con todas las canciones

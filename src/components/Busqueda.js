...
const Busqueda = ({canciones, currentSong, setCurrentSong, db, listas, setListas, nodeConverter, songsToAdd}) => {
            ...
            <div id="resultadosBusquedaContainer" style={searchWord.length !== 0 && matchedSongs.length !== 0 ? {height:(height + sumHeight).toString()+"em"}:null}>
                <div id = "resultadosBusquedaDiv" style ={searchWord.length !== 0 && matchedSongs.length !== 0 ? {height:(height).toString()+"em"}:null}>
                    ...
                    {cancionesSeleccionadas.length !== 0 ? 
                        <>
                            <AddListaForm db = {db} listas = {listas} setListas={setListas} nodeConverter={nodeConverter} songsToAdd={cancionesSeleccionadas} setCancionesSeleccionadas={setCancionesSeleccionadas} setHacerGrande={setHacerGrande} setSearchWord = {setSearchWord}></AddListaForm>

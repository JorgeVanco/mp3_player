...
const Song = ({
  ...
  const updateEscuchas = async (song_name, song_author) => {
    axios.post(API_URL + "/reproductions", {
      song_name: song_name,
      author: song_author,
      user: user?.email,
    });
  ...
  return (
    <div
      className={smallCard ? "smallCard" : "songCard"}
      onClick={() => toggleSmallCard(smallCard, setSmallCard, setTab)}
      style={
        hacerGrande
          ? {
              width: "100%",
    ...
    >
          ...
          <div className="smallCardControls">
            {repeatState === 2 ? (
              <RiRepeatOneLine
                size={28}
                onClick={(e) =>
                  changeRepeatState(e, setRepeatState, repeatState)
                }
                style={{ color: "green" }}
              ></RiRepeatOneLine>
            ) : (
              <RiRepeat2Line
                ...
                style={repeatState ? { color: "green" } : null}
              ></RiRepeat2Line>
            )}
            <MdSkipNext
              size={35}
            ...
            {isPaused ? (
              <FaPlay
                className="playButton smallCardPlayButton"
                onClick={(e) => playSong(e, audioRef, setIsPaused)}
              ></FaPlay>
            ) : !isPaused ? (
              <FaPause
                className="playButton smallCardPlayButton"
                onClick={(e) => pauseSong(e, audioRef, setIsPaused)}
              ></FaPause>
            ) : null}
          </div>
        </div>
      ...
      <div
        className={
          smallCard ? "progressBarContainerSmallCard" : "progressBarContainer"
        }
      >
        <div
          className="progressBar"
          ref={progressBarRef}
          onClick={(e) => progressBarClick(e, audioRef, progressBarRef)}
        >
          <div
      ...
      <div className={smallCard ? "hidden" : "controlSongDiv"}>
        {!smallCard ? (
          <TbRewindBackward10
            ...
            onClick={(e) => rewind(e, audioRef, -10)}
          ></TbRewindBackward10>
        ) : null}
        {!smallCard ? (
          <MdSkipPrevious
            size={42}
            className="skipButton skipPrevious"
            onClick={(e) =>
              handlePrev(e, currentSong, setCurrentSong, audioRef)
            }
          ></MdSkipPrevious>
        ) : null}
      ...
      {!smallCard ? (
        <>
          <IoShuffle
            ...
            onClick={(e) => shuffle(e, songList, setCurrentSong)}
          ></IoShuffle>
          {user ? (
            <AddListaForm
              db={db}
              ...
              songsToAdd={[currentSong]}
              setCancionesSeleccionadas={null}
              setHacerGrande={setHacerGrande}
              setReload={setReload}
            ></AddListaForm>
          ) : (
          ...
          {repeatState === 2 ? (
            <RiRepeatOneLine
              size={28}
              className={
                !smallCard ? "repeatIcon" : "repeatIcon repeatIconSmall"
              }
              onClick={(e) => changeRepeatState(e, setRepeatState, repeatState)}
              style={{ color: "green" }}
            ></RiRepeatOneLine>
          ) : (
            <RiRepeat2Line
